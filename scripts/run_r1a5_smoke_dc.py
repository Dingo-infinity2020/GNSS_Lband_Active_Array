"""R1A5 diagnostic HF frequency-domain smoke solve on NW."""
...
def run(repo,evidence,work,base_cst):
    ...
    os.makedirs(work)
    cfg=os.path.join(repo,"source","cst","R1A5_SMOKE_SOLVER_CONFIG_V01.mcr")
    dst=os.path.join(work,"R1A5_DIAGNOSTIC_SMOKE_V01.cst")
    ...
    summary={
      ...
      "mesh":"tetrahedral first order; adaptation off",
    ...
    with open(os.path.join(evidence,"harness_log.txt"),"w") as f:
        f.write("\n".join(log)+"\n")
    status="PASS_R1A5_DIAGNOSTIC_SMOKE" if checks["pass"] else "HOLD_R1A5_DIAGNOSTIC_INTEGRITY"
    print(status)
    print("R1A5_CST_PATH="+dst)
    print("R1A5_CST_SHA256="+summary["r1a5_cst_sha256"])
