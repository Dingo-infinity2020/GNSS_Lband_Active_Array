...
def protected_build_evidence(repo,variant,state):
    label=variant+"_"+state
    root=os.path.join(repo,"evidence","r1e1a2_support_build_nw_20260925_01",label)
    return [os.path.join(root,"build_shapes.txt"),os.path.join(root,"build_status.txt")]

def snapshot_files(paths):
    snap=[]
    for path in paths:
        if not os.path.isfile(path):
            raise RuntimeError("HOLD_R1E1A3R1_PROTECTED_EVIDENCE_MISSING: "+path)
        with open(path,"rb") as f:
            snap.append((path,f.read()))
    return snap
...
def restore_snapshot(snap):
    ...
    for path,data in snap:
        with open(path,"rb") as f:
            if f.read()!=data:
                raise RuntimeError("HOLD_R1E1A3R1_EVIDENCE_RESTORE_FAILED: "+path)
    return changed
...
def run(repo,evidence,work,source_cst,source_sha,variant,state):
    ...
    cfg=os.path.join(repo,"source","cst","R1E1A3R1_SUPPORT_SOLVER_CONFIG_V02.mcr")
    if not os.path.isfile(ref_path) or not os.path.isfile(cfg): raise RuntimeError("HOLD_R1E1A3R1_CONTEXT_MISSING")
    os.makedirs(evidence); os.makedirs(work)
    dst=os.path.join(work,"R1E1A3R1_%s_%s_SOLVE_V02.cst"%(variant,state))
    shutil.copy2(source_cst,dst)
    if sha(dst).lower()!=source_sha.lower(): raise RuntimeError("HOLD_R1E1A3R1_COPY_HASH_MISMATCH")

    protected_snapshot=snapshot_files(protected_build_evidence(repo,variant,state))
    protected_rewrites=[]
    ...
    try:
        ...
        prj.modeler.add_to_history("R1E1A3-R1 recovery solver configuration",macro_body(cfg))
        prj.modeler.add_to_history("R1E1A3-R1 pre-solver audit",audit_vba(pre))
        prj.save()
        st=parse_kv(pre)
        ...
        de.close()
        protected_rewrites=restore_snapshot(protected_snapshot)
    ...
    summary={"variant":variant,"state":state,"theta":theta,"phi":phi,
      ...
      "protected_build_evidence_rewrites_detected":protected_rewrites,
      "protected_build_evidence_restored":True,
    ...
    print(("PASS" if numerical["pass"] else "HOLD")+"_R1E1A3R1_"+variant+"_"+state+"_SOLVE")
