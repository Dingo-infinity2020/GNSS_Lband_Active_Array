"""R1E1A1 endpoint pitch-parameterization BUILD-ONLY harness.
...
ENDPOINTS=[("P088",88.0),("P090",90.0),("P092",92.0),("P094",94.0),("P096",96.0),("P100",100.0)]
SOLVER_MARKERS=(
  "meshing successful",
...
def audit_vba(shape_path,status_path,label):
    return "\n".join([
      ...
      'Print #f, "R1E1A1_%s_SHAPE_INVENTORY"'%label,
...
def run_one(repo,evidence,work,source,label,pitch,source_shapes):
    ...
    dst=os.path.join(work,"R1E1A1_%s_PITCH_BUILD_ONLY_V01.cst"%label)
    shutil.copy2(source,dst)
    pre_sha=sha(dst)
    if pre_sha!=SOURCE_SHA: raise RuntimeError("HOLD_R1E1A1_%s_COPY_HASH_MISMATCH"%label)
...
def run(repo,evidence,work,source):
    if os.path.exists(evidence): raise RuntimeError("HOLD_R1E1A1_EVIDENCE_DIR_ALREADY_EXISTS")
    if os.path.exists(work): raise RuntimeError("HOLD_R1E1A1_WORK_DIR_ALREADY_EXISTS")
    if not os.path.isfile(source): raise RuntimeError("HOLD_R1E1A1_SOURCE_MISSING")
    if sha(source)!=SOURCE_SHA: raise RuntimeError("HOLD_R1E1A1_SOURCE_HASH_MISMATCH")
    ...
    top={
      ...
      "pitch_count_6":len(results)==6,
      "all_six_pass":all(x["checks"]["pass"] for x in results),
      "artifact_hashes_unique":len(set(x["cst_sha256"] for x in results))==6,
    }
    top["pass"]=all(top.values())
    summary={"mode":"R1E1A1_SIX_PITCH_FR4_SOURCE_SET_BUILD_ONLY","solver_run":False,"endpoints":results,"checks":top}
    with open(os.path.join(evidence,"summary.json"),"w") as f: json.dump(summary,f,indent=2)
    print("PASS_R1E1A1_SIX_PITCH_FR4_SOURCE_SET_BUILD_ONLY" if top["pass"] else "HOLD_R1E1A1_SIX_PITCH_FR4_SOURCE_SET_BUILD_ONLY")
    for x in results:
