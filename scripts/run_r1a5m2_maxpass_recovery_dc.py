"""R1A5M2 max-pass recovery convergence qualification on NW."""
...
BASELINE_CSV_REL=os.path.join("evidence","r1a5m_dc_nw_20260924_adapt01","sparameters_and_zin.csv")
Z0=100.0
...
def parse_native_adaptation(cstfile):
    stem=os.path.splitext(cstfile)[0]
    p=os.path.join(stem,"Result","output.txt")
    if not os.path.isfile(p):
        raise RuntimeError("R1A5M2 output.txt missing")
    ...
    last_two_below=(len(seq)>=2 and seq[-1]["delta_s"]<=0.02 and seq[-2]["delta_s"]<=0.02)
    ...
    return {
      ...
      "last_two_below_0p02":last_two_below,
...
def run(repo,evidence,work,base_cst):
    ...
    os.makedirs(work)
    cfg=os.path.join(repo,"source","cst","R1A5M2_ADAPTIVE_MAXPASS8_CONFIG_V01.mcr")
    dst=os.path.join(work,"R1A5M2_ADAPTIVE_MAXPASS8_V01.cst")
    ...
    native=parse_native_adaptation(dst)
    ...
    checks={
      ...
      "baseline_s11_complex_converged":comparison["max_complex_delta_s11"]<=0.03,
      "baseline_s22_complex_converged":comparison["max_complex_delta_s22"]<=0.03,
      "native_two_consecutive_delta_s_pass":native["last_two_below_0p02"],
    ...
    summary={
      ...
      "baseline_csv_sha256":sha(baseline_path),
      "r1a5m2_cst":dst,
      "r1a5m2_cst_sha256":sha(dst),
      "r1a5m2_cst_bytes":os.path.getsize(dst),
      "solver":"HF Frequency Domain",
      "mesh":"tetrahedral second order adaptive; curvature order 3; general purpose",
      "adaptation":{"min_passes":3,"max_passes":8,"max_delta_s":0.02,"delta_s_checks":2,"linear_growth_limitation":40},
      "frequency_ghz":[1.0,1.8],
      "metrics":metrics,
      "comparison_to_r1a5m_pass6":comparison,
    ...
    native_ok=(
      checks["native_two_consecutive_delta_s_pass"]
      and checks["native_not_maxpass_terminated"]
      and checks["native_no_error_lines"]
      and checks["broadband_sweep_converged"])
    absolute_ok=(
      checks["baseline_s11_complex_converged"]
      and checks["baseline_s22_complex_converged"]
      and checks["pol_symmetry_pass"]
      and checks["reciprocity_pass"]
      and checks["all_curves_present"]
      and checks["no_nan_inf"])
    if checks["pass"]:
        status="PASS_R1A5M2_NATIVE_AND_ABSOLUTE_CONVERGED"
    elif not native_ok:
        status="HOLD_R1A5M2_NATIVE_NOT_CONVERGED"
    elif not absolute_ok:
        status="HOLD_R1A5M2_ABSOLUTE_NOT_CONVERGED"
    else:
        status="HOLD_R1A5M2_INTEGRITY"
    print(status)
    print("R1A5M2_CST_PATH="+dst)
    print("R1A5M2_CST_SHA256="+summary["r1a5m2_cst_sha256"])
    ...
    print("MAX_D_S11_VS_R1A5M_PASS6=%.9f"%comparison["max_complex_delta_s11"])
    print("MAX_D_S22_VS_R1A5M_PASS6=%.9f"%comparison["max_complex_delta_s22"])
    print("NATIVE_DELTA_SEQUENCE="+json.dumps(native["delta_s_sequence"]))
    print("NATIVE_MAXPASS_REACHED="+str(native["max_passes_reached"]))
