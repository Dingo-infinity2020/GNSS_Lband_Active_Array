"""R1E1A0R2 endpoint pitch-parameterization BUILD-ONLY harness.
...
def audit_vba(shape_path,status_path,label):
    return "\n".join([
      ...
      'Print #f, "R1E1A0R2_%s_SHAPE_INVENTORY"'%label,
...
def run_one(repo,evidence,work,source,label,pitch,source_shapes):
    ...
    dst=os.path.join(work,"R1E1A0R2_%s_PITCH_BUILD_ONLY_V01.cst"%label)
    shutil.copy2(source,dst)
    pre_sha=sha(dst)
    if pre_sha!=SOURCE_SHA: raise RuntimeError("HOLD_R1E1A0R2_%s_COPY_HASH_MISMATCH"%label)
...
def run(repo,evidence,work,source):
    if os.path.exists(evidence): raise RuntimeError("HOLD_R1E1A0R2_EVIDENCE_DIR_ALREADY_EXISTS")
    if os.path.exists(work): raise RuntimeError("HOLD_R1E1A0R2_WORK_DIR_ALREADY_EXISTS")
    if not os.path.isfile(source): raise RuntimeError("HOLD_R1E1A0R2_SOURCE_MISSING")
    if sha(source)!=SOURCE_SHA: raise RuntimeError("HOLD_R1E1A0R2_SOURCE_HASH_MISMATCH")
    ...
    summary={"mode":"R1E1A0R2_PITCH_PARAMETERIZATION_BUILD_ONLY","solver_run":False,"endpoints":results,"checks":top}
    with open(os.path.join(evidence,"summary.json"),"w") as f: json.dump(summary,f,indent=2)
    print("PASS_R1E1A0R2_PITCH_PARAMETERIZATION_BUILD_ONLY" if top["pass"] else "HOLD_R1E1A0R2_PITCH_PARAMETERIZATION_BUILD_ONLY")
    for x in results:
